...
2. Construct  the communicator according to rank information from mpi. In this example, we  utilize  the  python  package mpi4py [Source](https://mpi4py.readthedocs.io/en/stable/intro.html) to  determine  the  local rank ID when the application has been launched with mpirun. All configuration information is stored in the FPGA so that the CCLO can rapidly access them. 
