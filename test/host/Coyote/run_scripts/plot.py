...
def read_mpi_host_log_files(log_dir):
    ...
    # Group by specified columns and calculate the mean for specific columns
    grouped_df = mpi_dataframes_con.groupby(["collective", "count", "number_of_nodes", "rank_id"]).mean()
...
# Generate plots
def generate_time_size_plots(accl_dataframes, mpi_dataframes, output_dir, node_value):
    ...
    for collective in collective_values:
        ...
        if collective == 'sendrecv':
            ...
            # plt.title(f'Throughput vs Size for {collective}',fontsize=18)
...
if __name__ == "__main__":
    ...
    mpi_host_log_dir = "./mpi-benchmarking/results"
    ...
    mpi_host_dataframes = read_mpi_host_log_files(mpi_host_log_dir)
    print(mpi_host_dataframes.head(200))
    # print(mpi_dataframes)
    # print(accl_dataframes)
    generate_time_size_plots(accl_dataframes, mpi_dataframes, output_dir, 8)
    generate_time_size_plots(accl_dataframes, mpi_dataframes, output_dir, 4)
    generate_time_nodes_plots(accl_dataframes, mpi_dataframes, output_dir, 64)
    generate_time_nodes_plots(accl_dataframes, mpi_dataframes, output_dir, 128)
    generate_time_nodes_plots(accl_dataframes, mpi_dataframes, output_dir, 256)
